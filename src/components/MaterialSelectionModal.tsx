...
const MaterialSelectionModal: React.FC<MaterialSelectionModalProps> = ({
  ...
  const updateLineSearch = (index: number, value: string) => {
    ...
    if (value.length >= 2) {
      ...
      const matching = products.filter(p =>
        ...
        p.gb_article_number.toLowerCase().includes(value.toLowerCase()) ||
        (p.ean && p.ean.toLowerCase().includes(value.toLowerCase()))
      );

      // Auto-select if exact match
      if (matching.length === 1 || matching.some(p => p.ean === value || p.gb_article_number === value)) {
        const exactMatch = matching.find(p => p.ean === value || p.gb_article_number === value) || matching[0];
  ...
  const getFilteredProducts = (searchValue: string) => {
    ...
    return products.filter(p =>
      ...
      p.gb_article_number.toLowerCase().includes(searchValue.toLowerCase()) ||
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 relative">
                      <input
                        ...
                        placeholder="Zoek op naam, GB-art.nr. of scan barcode"
                          ...
                          {getFilteredProducts(line.searchValue).map((product) => (
                            ...
                            >
                              ...
                              <div className="text-sm text-gray-600">
                                GB-art.nr.: {product.gb_article_number} | {product.category} | {product.unit}
                              </div>
                            </button>
                          ))}
                  ...
                  {searchResults.map((product) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600 mt-1">
                        GB-art.nr.: {product.gb_article_number} | Categorie: {product.category} | Eenheid: {product.unit}
                      </div>
                      {product.ean && (
