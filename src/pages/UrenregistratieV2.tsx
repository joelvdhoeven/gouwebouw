                      ...
                      <tr key={reg.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">
                          {(() => {
                            ...
                            // Check if it's in "CODE - Name" format (split on first " - ")
                            const separatorIndex = werktype.indexOf(' - ');
                            if (separatorIndex > 0) {
                              const code = werktype.substring(0, separatorIndex).trim();
                              const name = werktype.substring(separatorIndex + 3).trim();
                              return (
                                <span title={name}>
                                  <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-xs mr-1">{code}</span>
